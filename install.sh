...
#
# Runs only from a signed release that root itself cloned into a directory only root can write to:
#
#   sudo git clone --depth 1 --branch vX.Y.Z https://github.com/rogertobler/omarchy-tun0-vpn /usr/local/lib/tun0-vpn
#   sudo /usr/local/lib/tun0-vpn/install.sh [NAME FILE.ovpn ...]
#
# Later releases come through /usr/local/bin/vpn-update, which checks the new tag against the key pinned here before
# anything of that release runs. NAME = <provider>-<country>, one dash, e.g. proton-de ~/Downloads/de.protonvpn.udp.ovpn
# (optional, `vpn add` does the same later). Idempotent: run it again at any time.
#
# Nothing this script installs comes from a place anyone but root can write to, and nothing is changed before all of
# this has passed:
...
#   - the checkout is the unmodified commit of the release tag v<version>, that tag is signed by the release key
#     below, and that key is the one pinned in /etc/vpn/release-signer (written at the first installation); the
#     release is not older than the one recorded as installed
...
#   - the clone was made with --branch v<version>, so the release installed is the one the clone command named
...
on_exit() {
  ...
  [[ -n "$STAGE" ]] && rm -rf "$STAGE"
  if (( REVOKED )); then
    echo "install.sh: stopped before the end. The sudoers rule stays removed until install.sh runs through, so the panel and the vpn command cannot change the kill switch meanwhile; the kill switch itself keeps working. Run it again." >&2
  fi
  exit "$rc"
}
trap on_exit EXIT
...
# The public half of the key that signs every release tag. The same key is listed as a signing key of the GitHub
...
# Everything root installs from the tree, and the widget files the user side gets. Each is listed in SHA256SUMS.
PAYLOADS=(vpn-root 90-vpn-killswitch vpn-killswitch.service killswitch.nft.in sudoers-vpn.in uninstall.sh vpn-update vpn)
...
if [[ "$self" != "$TREE/install.sh" ]]; then
  ...
  cat >&2 <<'EOF'
install.sh: this runs as root only from a release that root cloned itself, never from a folder you can write to
(a program running as you could change this script, or a file it installs, while sudo runs it). Install with the
release named in the README (vX.Y.Z):

  sudo git clone --depth 1 --branch vX.Y.Z https://github.com/rogertobler/omarchy-tun0-vpn /usr/local/lib/tun0-vpn
  sudo /usr/local/lib/tun0-vpn/install.sh
...
(( ${#missing[@]} == 0 )) || die "not installed: ${missing[*]}. Install with  sudo pacman -S --needed ${missing[*]}  and run this again. Nothing was changed."
...
[[ -n "$sudo_version" && "$(printf '1.9.10\n%s\n' "$sudo_version" | sort -V | head -1)" == 1.9.10 ]] \
  || die "sudo ${sudo_version:-of unknown version} is older than 1.9.10, which the sudoers rule needs (regular expressions in arguments). Update sudo and run this again. Nothing was changed."

# Which release. The version comes from the commit, not from the working tree.
...
tagged=$(tgit rev-parse --verify --quiet "refs/tags/$tag^{commit}") || die "tag $tag is missing (clone with --branch $tag)"
[[ "$head" == "$tagged" ]] || die "the checkout is $head, not the release $tag ($tagged)"
[[ -z "$(tgit status --porcelain --untracked-files=all --ignored)" ]] || die "$TREE differs from the release $tag (git -C $TREE status --ignored)"
# Every tag in the clone names itself as it is called. A signed tag served under another name (v1.1.1 as v1.1.9)
# would otherwise install a different release than the one the clone command asked for.
tags=$(tgit for-each-ref refs/tags --format='%(refname:strip=2) %(objecttype) %(tag)')
while read -r tname ttype theader; do
  [[ -n "$tname" ]] || continue
  [[ "$ttype" == tag && "$theader" == "$tname" ]] || die "the clone holds the tag $tname, which is ${theader:+the tag $theader under another name}${theader:-not a signed tag}. Remove $TREE and clone again. Nothing was changed."
done <<<"$tags"
# The clone asked for exactly this tag. A branch, or an unsigned tag, named like a newer release but sitting on the
# commit of an older one would otherwise install that older release under the name the clone command asked for.
fetch=$(tgit config --get-all remote.origin.fetch || true)
[[ "$fetch" == "+refs/tags/$tag:refs/tags/$tag" ]] || die "the clone was not made with --branch $tag (it follows ${fetch:-nothing}), so it is not certain which release you asked for. Remove $TREE and clone again as the README says. Nothing was changed."
if [[ -e "$PINNED" ]]; then
  root_only "$PINNED" || die "$PINNED must belong to root and be writable by root only"
  [[ "$(cat "$PINNED")" == "$RELEASE_SIGNER" ]] || die "$tag names a different release key than the one pinned at the first installation ($PINNED). Nothing was changed. If the key really changed, compare both with the README on GitHub and remove $PINNED by hand."
fi
if [[ -e "$RECORD" ]]; then
...
tgit -c gpg.format=ssh -c gpg.ssh.allowedSignersFile="$STAGE/allowed_signers" verify-tag "$tag" >/dev/null 2>&1 \
  || die "tag $tag is not signed by the release key (git -C $TREE verify-tag $tag)"
echo "  $tag at $head, signed by the release key"
...
# from an empty environment. Without it `omarchy plugin enable` failed on every installation and the rescan was a quiet
# no-op behind `-q` (found at Doorman on 16 September 2026, the same on this installer).
...
# again - quickshell aborts ("Tried to show lockscreen surfaces without active lock"; Omarchy 4.0.2). Refuse to
...
absent() { [[ ! -e "$1" && ! -L "$1" ]]; }
foreign=()
for f in /etc/sudoers.d/vpn "$ROOT" /usr/local/bin/vpn-update /usr/local/bin/vpn-uninstall "$ETC/killswitch.nft.in" \
...
(( ${#foreign[@]} == 0 )) || die "not put there by tun0 VPN, so left alone: ${foreign[*]}. If it is yours to replace, move it out of the way and run this again. Nothing was changed."
...
user_can_create "$HOME_DIR/.local/bin" || die "$USER_NAME cannot write $HOME_DIR/.local/bin (the vpn command goes there). Fix its owner, e.g. sudo chown $USER_NAME: $HOME_DIR/.local/bin, and run this again. Nothing was changed."
PLUGIN_DIR="$HOME_DIR/.config/omarchy/plugins/$PLUGIN_ID"
WIDGET_GIT=0; as_user /usr/bin/test -d "$PLUGIN_DIR/.git" && WIDGET_GIT=1
if (( ! WIDGET_GIT )); then
  user_can_create "$PLUGIN_DIR" || die "$USER_NAME cannot write $PLUGIN_DIR (the widget goes there). Fix its owner and run this again. Nothing was changed."
fi
...
echo "== 2/6 Root helper, dispatcher, kill switch unit, updater (from the checked copies); the sudoers rule is removed first"
...
install -D -o root -g root -m755 "$STAGE/vpn-update" /usr/local/bin/vpn-update
...
if (( WIDGET_GIT )); then
  ...
  if [[ -z "$pv" || "$(printf '%s\n%s\n' "$pv" "$version" | sort -V | head -1)" != "$version" ]]; then
    ...
    echo "  widget is $pv, newer than this release $version: update the root side with  sudo /usr/local/bin/vpn-update v$pv"
  fi
elif screen_locked; then
  echo "  the screen was locked meanwhile: widget files not written (a hot reload under the lock crashes the shell). Run this again after unlocking." >&2
else
...
if [[ ! -e "$PINNED" ]]; then
  ...
  echo "  release key pinned in $PINNED: ${fp:-fingerprint unknown}. vpn-update accepts only releases signed by it."
  echo "  This first installation trusted the tag it cloned; compare that fingerprint with the README and with"
  echo "  https://api.github.com/users/rogertobler/ssh_signing_keys before you rely on updates."
fi
